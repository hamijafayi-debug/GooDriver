...
func (s *SOCKSServer) Serve(ctx context.Context) error {
	// Refuse to start an unauthenticated SOCKS proxy on a non-loopback address.
	// Doing so would expose an open proxy to the entire local network.
	// If LAN access is intentional, the caller must set AllowLANListen = true.
