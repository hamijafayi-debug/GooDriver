...
// Close cancels any in-flight background token refresh. It is safe to call
// multiple times. The AccessTokenSource must not be used after Close returns.
...
func (s *AccessTokenSource) refreshInBackground() {
	token, err := s.auth.accessTokenForRoute(s.ctx, s.route)
	s.mu.Lock()
	defer s.mu.Unlock()
	s.refreshing = false
	if err != nil {
...
func (a AuthConfig) tokenFromCommand(ctx context.Context) (string, error) {
	...
	// Warn if the command contains shell metacharacters that could enable
	// injection if token_command is populated from untrusted input.
	if strings.ContainsAny(command, "`$(){};&|<>") {
		log.Printf("WARNING: token_command contains shell metacharacters — ensure this value comes from a trusted source")
	}
