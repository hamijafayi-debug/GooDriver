...
func TestMuxMarkSeenCompactionPreservesQueuedClaims(t *testing.T) {
	// All entries are fresh (seenAt ≈ now). The 10-minute and 2-minute TTL
	// passes will not remove them. The last-resort reset is the only path
	// that fires. We verify it keeps the newly-seen entry and the queued claim.
	mux := &driveMux{
		seen:   map[string]seenEntry{},
		queued: map[string]struct{}{"inflight": struct{}{}},
	}
	for i := 0; i <= 200000; i++ {
		mux.seen[fmt.Sprintf("seen-%d", i)] = seenEntry{seenAt: time.Now()}
	}
	mux.markSeen("done")
...
func TestMuxMarkSeenTTLEvictsStaleEntries(t *testing.T) {
	// Populate seen with entries older than 10 minutes. The first TTL pass
	// must evict them, so the map stays well below 200k and the last-resort
	// reset is never reached. Entries that are still recent must survive.
	...
	// Fill with 200001 stale entries.
	for i := 0; i <= 200000; i++ {
		mux.seen[fmt.Sprintf("stale-%d", i)] = seenEntry{seenAt: old}
	}
	// Add a few recent entries that must survive.
	for i := 0; i < 5; i++ {
		mux.seen[fmt.Sprintf("recent-%d", i)] = seenEntry{seenAt: recent}
	}

	mux.markSeen("new-entry")

	// Stale entries must be gone.
	if _, ok := mux.seen["stale-0"]; ok {
		t.Error("stale entry survived TTL eviction")
	}
	// Recent entries must survive.
	for i := 0; i < 5; i++ {
		if _, ok := mux.seen[fmt.Sprintf("recent-%d", i)]; !ok {
			t.Errorf("recent entry recent-%d was incorrectly evicted", i)
		}
	}
	// The newly-marked entry must always survive.
	if _, ok := mux.seen["new-entry"]; !ok {
		t.Error("newly marked entry was lost after TTL eviction")
	}
	// Map must have shrunk — the last-resort catastrophic reset must NOT have fired.
	// If it had, only "new-entry" would remain (len == 1).
	// After eviction we expect recent entries (5) + new-entry (1) = 6 entries.
	if got := len(mux.seen); got > 10 {
		t.Errorf("seen map has %d entries after TTL eviction, expected ≤10 (stale entries not removed)", got)
	}
}

func TestMuxMarkSeenTTLSecondPassEvictsMidAgeEntries(t *testing.T) {
	// Entries between 2 and 10 minutes old are not caught by the first pass
	// (10-min cutoff) but are caught by the second pass (2-min cutoff).
	midAge := time.Now().Add(-5 * time.Minute)
	...
	for i := 0; i <= 200000; i++ {
		mux.seen[fmt.Sprintf("mid-%d", i)] = seenEntry{seenAt: midAge}
	}

	mux.markSeen("trigger")

	if _, ok := mux.seen["mid-0"]; ok {
		t.Error("mid-age entry survived second-pass TTL eviction")
	}
	if _, ok := mux.seen["trigger"]; !ok {
		t.Error("trigger entry was lost after second-pass TTL eviction")
	}
}
...
func TestNewTunnelPropagatesRoleFromConfig(t *testing.T) {
	store := NewMemoryStore()
	for _, tc := range []struct {
		role     string
		wantRole string
	}{
		{role: "client", wantRole: "client"},
		{role: "exit", wantRole: "exit"},
		{role: "", wantRole: ""},
		{role: "unknown", wantRole: ""},
	} {
		cfg := &Config{
			Secret: "test-secret",
			Role:   tc.role,
		...
		cfg.ApplyDefaults()
		tunnel, err := NewTunnel(store, cfg)
		if err != nil {
			t.Fatalf("role=%q: NewTunnel: %v", tc.role, err)
		}
		if tunnel.role != tc.wantRole {
			t.Fatalf("role=%q: tunnel.role = %q, want %q", tc.role, tunnel.role, tc.wantRole)
		}
	}
}
