...
func nonce(sid [16]byte, direction byte, sequence uint64) []byte {
	out := make([]byte, 12)
	// XOR-fold all 16 bytes of the session ID into the 5-byte prefix so that
	// every bit of the SID contributes to nonce uniqueness.
	// Each output byte XORs three SID bytes (indices spaced by 5 and 10) so
	// no byte is left out. The previous implementation omitted sid[5..10].
	// BREAKING WIRE-FORMAT CHANGE (introduced with XOR-fold nonce):
	// Both the client and the exit node must run the same version of this
	// function. A mixed-version pair will fail to decrypt each other's frames.
	// When deploying, upgrade exit nodes and clients atomically, or coordinate
	// a maintenance window. Existing in-flight sessions must be restarted after
	// both sides are upgraded.
	out[0] = sid[0] ^ sid[5] ^ sid[10]
	out[1] = sid[1] ^ sid[6] ^ sid[11]
	out[2] = sid[2] ^ sid[7] ^ sid[12]
	out[3] = sid[3] ^ sid[8] ^ sid[13]
	out[4] = direction ^ sid[4] ^ sid[9] ^ sid[14] ^ sid[15]
	for i := 0; i < 7; i++ {
