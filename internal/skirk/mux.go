...
func (m *driveMux) markSeen(name string) {
	...
	if len(m.seen) > 200000 {
		// First pass: remove entries older than 10 minutes. Drive objects are
		// typically deleted within minutes, so anything older can safely be forgotten.
		...
		// Second pass: if we are in a high-traffic burst where all 200k+ entries
		// are under 10 minutes old, fall back to a 2-minute TTL to avoid
		// unbounded growth. A 2-minute-old object is almost certainly gone from Drive.
		if len(m.seen) > 200000 {
			cutoff2 := now.Add(-2 * time.Minute)
			for k, v := range m.seen {
				if v.seenAt.Before(cutoff2) {
					delete(m.seen, k)
				}
			}
		}
		// Last resort: catastrophic reset to avoid OOM.
		if len(m.seen) > 200000 {
