...
- Fixed `tokenNeedsRefreshForRoute` to use a single `lifetime <= margin` condition,
  removing a redundant second check that was mathematically equivalent for all
  non-boundary inputs. Added table-driven tests covering expired, boundary, and
  within/outside-margin cases for both direct and hostile route margins.
- Removed redundant `net.Dialer.Timeout` from `dialDirectExitTarget`. The caller
  (`dialExitTarget`) already wraps the context with `context.WithTimeout`, making
  the dialer-level timeout a duplicate with no effect. The context deadline is now
  the sole authoritative timeout.
- Removed unused `Plane` field from `muxObjectMeta`. The field was never set or
  read anywhere in the codebase and was left over from an earlier experiment.
- Fixed type mismatch in `tunnel_test.go`: all `driveMux` test literals that
  initialized `seen` as `map[string]struct{}{}` were updated to `map[string]seenEntry{}`
  to match the TTL-aware seen-map type introduced alongside the two-pass eviction fix.
  Without this fix the test package would fail to compile.
- Replaced the single `TestMuxMarkSeenCompactionPreservesQueuedClaims` test with
  three targeted tests that cover: last-resort reset preserving queued claims (existing
  behavior), first-pass TTL eviction of entries older than 10 minutes, and second-pass
  eviction of mid-age entries (2–10 minutes old). The new tests verify that TTL eviction
  removes stale entries without discarding recent ones, guarding against regression to
  the catastrophic full-map reset that existed in the upstream version.
